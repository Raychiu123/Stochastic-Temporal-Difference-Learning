...
for ep in range(epoch):
    for split in splits:
        ...
        for itr, (_, dec_inputs, targets, lengths) in enumerate(dataloader):
            ...
            model.forward(dec_inputs, lengths, drop = True)
            ...
            if split == 'train':
                ...
                for _ in range(small_epoch):
                    # forward
                    model(dec_inputs, lengths, drop = False)
                    t_loss, t_nll, t_over, kl_loss = model.calculate_loss(targets)

                    t_loss = t_loss / bsize
                    t_nll = t_nll / bsize
                    t_over = t_over / bsize
                    kl_loss = kl_loss / bsize
                ...
                tracker['KL'].append(kl.item())
                #tracker['KL'].append(0)

                if itr % print_every == 0 or itr + 1 == len(dataloader):
                    print("%s Batch %04d/%04d, NLL-Loss %.4f, model_loss %.4f, tdvae_nll %.4f, overshooting_nll %.4f, kl_loss %.4f"
                           %(split.upper(), itr, len(dataloader), tracker['NLL'][-1], loss, nll, over, tracker['KL'][-1]))
                    print("Epoch %d, %s Batch %04d/%04d, NLL-Loss %.4f, KL-Loss %.4f"
                           %(ep, split.upper(), itr, len(dataloader), tracker['NLL'][-1], tracker['KL'][-1]),
        ...
        model.eval()
        model.forward(dec_inputs, lengths, drop = False)
        z = model.z_compare()
        # save checkpoint
        if split == 'train':
