...
class TD_VAE(nn.Module):
    ...
    def rollout(self):
        ...
        #for i in range(self.b.size()[1]):
            #z_mu, z_logsigma = self.b_to_z(self.b[:,i,:])
            #z_epsilon = torch.randn_like(z_mu)
            #z = z_mu + torch.exp(z_logsigma)*z_epsilon
            ## predicting states after time t1 using state transition

            #h, _ = self.transition_z(z.view(self.batch_size, self.b.size()[1], -1))#.view(self.batch_size, ))

            #next_z_mu, next_z_logsigma = self.transition_z(z)
            #next_z_mu, next_z_logsigma = self.b_to_z(h)
            #next_z_epsilon = torch.randn_like(next_z_mu)
            #next_z = next_z_mu + torch.exp(next_z_logsigma)*next_z_epsilon
        ...
        #loss = NLL(F.log_softmax(t3_x_prob), target[:,t3], self.length + 1)
