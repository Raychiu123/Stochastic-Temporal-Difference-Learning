...
class STDL(nn.Module):
    def __init__(self, x_size, processed_x_size, b_size, z_size):
        ...
        ## infer state
        self.infer_z = DBlock(4*b_size + 3*z_size, 50, z_size)
    ...
    def calculate_loss(self, t1, t2, t3, t4):
        t4_z_mu, t4_z_logsigma = self.b_to_z(self.b[:, t4, :])
        t4_z_epsilon = torch.randn_like(t4_z_mu)
        t4_z = t4_z_mu + torch.exp(t4_z_logsigma)*t4_z_epsilon

        t3_qs_z_mu, t3_qs_z_logsigma = self.infer_z(
            torch.cat((t4_z.new_zeros(self.b[:,t1,:].size()),
                       t4_z.new_zeros(self.b[:,t2,:].size()),
                       self.b[:,t3,:],self.b[:,t4,:], 
                       t4_z.new_zeros(t4_z.size()),t4_z.new_zeros(t4_z.size()),t4_z), dim = -1))
        ...
        t2_qs_z_mu, t2_qs_z_logsigma = self.infer_z(
            torch.cat((t3_z.new_zeros(self.b[:,t1,:].size()),self.b[:,t2,:],self.b[:,t3,:],self.b[:,t4,:], 
                       t3_z.new_zeros(t3_z.size()),t3_z,t4_z), dim = -1))
        ...
        t1_qs_z_mu, t1_qs_z_logsigma = self.infer_z(
            torch.cat((self.b[:,t1,:],self.b[:,t2,:],self.b[:,t3,:],self.b[:,t4,:], 
                       t2_z, t3_z, t4_z), dim = -1))
        ...
        loss += gaussian_log_prob(t4_z_mu, t4_z_logsigma, t4_z)
        ...
        loss += -torch.sum(self.x[:,t4,:]*torch.log(t4_x_prob) + (1-self.x[:,t4,:])*torch.log(1-t4_x_prob), -1)
